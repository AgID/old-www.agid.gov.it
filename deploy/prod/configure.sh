...
read -p "Provide IP or FQDN of the target server [site.agid.local]: " HOST
if [ -z "$HOST" ]; then
  HOST=site.agid.local;
fi
...
sed "s#@HOST@#${HOST}#g" hosts.template > hosts
echo "Deploy host generated with provided parameters."